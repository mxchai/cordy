...
module.exports = function(babel) {
   ...
   */
  function chainBinaryOr(arrElements, scope) {
   ...
   */
  function createTaintStatusUpdate(path, lhsName, rhsTaint, scope="") {
   ...
   * @return {null}                   Instruments the code, no return value
   */
  function handleVariableDeclarator(varDeclarator, path, scope) {
    var rhs = varDeclarator.init;
    var lhsName = varDeclarator.id.name;

      //////////// Identifier ////////////
    if (t.isIdentifier(rhs)) {
      var rhsTaint = getTaint(rhs, scope);
      createTaintStatusUpdate(path, lhsName, rhsTaint, scope);
  ...
  function instrumentVariableDeclaration(path) {
    ...
    for (var i = 0; i < arrLength; i++) {
      ...
      if (t.isBlockStatement(path.parent)) {
        ...
        scope = "hey";
        handleVariableDeclarator(declarator, path, funcDeclaration.id.name);
        scope = "";
      } else {
